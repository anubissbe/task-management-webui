...
export function Board() {
  ...
  const TaskCard = ({ task, isSubtask = false }: { task: Task; isSubtask?: boolean }) => {
    ...
    return (
      <div className={`bg-white dark:bg-gray-700 p-3 rounded-lg shadow-sm border transition-all ${
        ...
        <div className="h-full flex gap-4 overflow-x-auto">
          {TASK_STATUSES.map((status) => {
            ...
            return (
              <div
                key={status}
                className={`min-w-[320px] flex-1 ${STATUS_COLORS[status]} dark:bg-gray-800 rounded-lg border-2 dark:border-gray-600 flex flex-col h-full`}
              >
