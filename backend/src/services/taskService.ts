...
import { Task, TaskStatus, TaskPriority, TaskHistory, TestResult } from '../types';

export class TaskService {
  async getTasksByProject(projectId: string, status?: TaskStatus): Promise<Task[]> {
    let query = `
      SELECT t.*, 
  ...
  async updateTaskStatus(id: string, status: TaskStatus, notes?: string): Promise<Task | null> {
    ...
    try {
      ...
      if (notes) {
        await client.query(
          `INSERT INTO project_management.task_history (task_id, action, new_value, notes)
